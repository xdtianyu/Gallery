...
public class MediaFile implements Comparable<MediaFile> {
    ...
    public final static String TAG = MediaFile.class.getSimpleName();
    ...
    public MediaFile(String path) throws MalformedURLException {
        this(path, getSmbHost(path));
    }

    public MediaFile(String path, String host) throws MalformedURLException {
        if (cachePath == null) {
    ...
    public MediaFile(SmbFile smbFile) {
        this.smbFile = smbFile;
        initMedia();
    }

    public MediaFile(WebDavFile davFile) {
        this.davFile = davFile;
        initMedia();
    }

    public MediaFile(File localFile) {
        this.localFile = localFile;
        initMedia();
    }

    public static MediaFile fromUri(String uri) {
        try {
    ...
    public MediaFile[] listMedia() throws SmbException {
        ArrayList<MediaFile> list = new ArrayList<>();
        if (smbFile != null) {
            SmbFile[] files = smbFile.listFiles();
            for (SmbFile file : files) {
                if (!file.getName().contains(":")) {
                    list.add(new MediaFile(file));
                }
            }
        } else if (localFile != null) {
        ...
        return list.toArray(new MediaFile[list.size()]);
    }
    ...
    public boolean equals(Object object) {
        ...
        if (object != null && object instanceof MediaFile) {
            equal = this.getPath().equals(((MediaFile) object).getPath());
        }
    ...
    @Override
    public int compareTo(@NonNull MediaFile another) {
